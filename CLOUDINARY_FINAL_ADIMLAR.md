...
## 🔧 ADIM 1: UNSIGNED UPLOAD PRESET OLUŞTUR

### Cloudinary Dashboard'da:

1. **Settings** (⚙️) → **"Upload"** sekmesine git
2. **"Add upload preset"** butonuna tıkla
3. Ayarlar:
   - **Preset name:** `fifa-match-tracker` (veya istediğin isim, örn: `fifa`)
   - **Signing mode:** **"Unsigned"** seç ⚠️ (ÖNEMLİ!)
   - **Folder (optional):** `fifa-matches` (opsiyonel)
   - Diğer ayarları varsayılan bırak
4. **"Save"** butonuna tıkla

**Preset name'i not al!** (örn: `fifa-match-tracker` veya `fifa`)

---

## 🚀 ADIM 2: RAILWAY'E ENVIRONMENT VARIABLES EKLE
...
5. **Key:** `VITE_CLOUDINARY_UPLOAD_PRESET`
   **Value:** `fifa-match-tracker` (veya oluşturduğun preset name)
   **"Add"** tıkla
...
- **API Secret güvenli tutulmalı!** Sadece backend'de kullanılır!

---

**Upload Preset oluşturdun mu? Railway'e ekledikten sonra "tamam" yaz, redeploy yapalım! 🚀**
